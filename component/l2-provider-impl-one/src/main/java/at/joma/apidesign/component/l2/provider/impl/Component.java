...
public class Component implements IL2Component {
	...
	public boolean isValid() {
		...
		Query<ValidOptionsTuple> isValidQuery = and(QueryFactory.in(ValidOptionsTuple.VOT_SORTINGORDER, sortingOrder),
				QueryFactory.in(ValidOptionsTuple.VOT_SORTINGDIRECTION, sortingDirection));

		isValid = VALIDOPTIONSTUPLES.retrieve(isValidQuery).size() >= 1;

		if (isValid) {
			isValid = configuredOptions.get().isValueSet(Omitting.BYFIELDNAMES_OPTIONNAME) //
					&& configuredOptions.get().isValueSet(Omitting.BYFIELDANNOTATIONS_OPTIONNAME) //
					&& configuredOptions.get().isValueSet(Omitting.BYFIELDCLASSES_OPTIONNAME) //
			;
		}
	...
	public void initializeSerializingInstance() {
		...
		if (SortingOrder.ALPHABETICALLY == options.getValueFor(SortingOrder.class)) {
			this.configuredXStreamInstance = new XStream(new PureJavaReflectionProvider(new FieldDictionary(new AlphabetKeySorter(options.getValueFor(SortingDirection.class))))) {
