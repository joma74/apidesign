...
public class ConfiguredOptionsHolder implements IConfiguration, Serializable {
	...
	public static final String MESSAGE_FAILURE_ASKCONFIGIUREDENUMOPTIONBYENUM = "The configured option value for your type enum must be asked via getValueFor(Class<T> enumType) only. Applies for ";

	private final StringArrClassArrConverter STRING_ARR_CLASS_ARR_CONVERTER = new StringArrClassArrConverter();
	...
	public Object getValueFor(String optionName) {
		...
		if (conf != null) {
			...
			if (conf.type == OptionType.STRING && optionName.equals(conf.name)) {
				return conf.getValue();
			}
			if (conf.type == OptionType.CLAZZARRAY && optionName.equals(conf.name)) {
				return STRING_ARR_CLASS_ARR_CONVERTER.convert(conf.convertValueToArray());
			}
		}
	...
	public ConfiguredOptionsHolder with(String optionName, Class<?>[] optionValue) {
		Option conf = new Option(optionName, optionValue);
		this.configuredOptions.put(optionName, conf);
		return this;
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof ConfiguredOptionsHolder) {
			...
			return Objects.equal(configuredOptions, other.configuredOptions) && Objects.equal(formatInfos, other.formatInfos);
		} else {
			return false;
		}
	}
